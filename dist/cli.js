...
import { setUserPreference, setHouseholdPreference, getUserPreferences, getHouseholdPreferences, clearUserPreference, clearHouseholdPreference, } from "./preferences.js";
import { browseLibrary } from "./browse.js";
import { searchMusic } from "./search.js";
import { setMood, getMood, listMoods, deleteMood } from "./mood.js";
import { getRecentHistory, listSessions, addAvoidTrack, removeAvoidTrack, getAvoidList, } from "./history.js";
import { playMedia, pauseMedia, stopMedia, nextTrack, prevTrack, setVolume, addToQueue, } from "./playback.js";
function usage() {
...
function getFlagInt(argv, name, def) {
...
function getFlagFloat(argv, name) {
    const v = getFlag(argv, name);
    if (!v)
        return undefined;
...
async function main() {
    ...
    if (cmd === "play") {
        ...
        const enqueue = getFlag(argv, "--enqueue");
        if (!speaker || !uri)
            usage();
        const client = HaClient.fromEnv();
    ...
    try {
        ...
        if (cmd === "prefs") {
            ...
            if (sub === "set") {
                ...
                if (!score || (!userSlug && !householdSlug))
                    usage();
                if (score < -1.0 || score > 1.0) {
                    throw new Error("Score must be between -1.0 and 1.0");
                }
                // Determine entity type and ID
                const entityTypes = ["track", "artist", "album", "genre", "decade", "year"];
                let entityType;
                let entityId;
        ...
        if (cmd === "mood") {
            ...
            if (sub === "set") {
                ...
                if ((!userSlug && !householdSlug) || !name)
                    usage();
                ...
                if (genres)
                    criteria.genres = genres.split(",").map((g) => g.trim());
                if (decades)
                    criteria.decades = decades.split(",").map((d) => d.trim());
                if (artists)
                    criteria.artists = artists.split(",").map((a) => a.trim());
